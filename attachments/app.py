...
# --- 7. STUDENT VIEW ---
    ...
    t_ai, t_port, t_road, t_export = st.tabs(["💬 Achievement Journalist", "📂 Portfolio & Manual Entry", "🤖 Career Mentor", "📤 Export"])
    ...
    # Generate ZIP data using the button format you requested
    portfolio_zip = db.get_full_portfolio_zip(st.session_state.user)

    st.download_button(
        label="Download Portfolio as ZIP",
        data=portfolio_zip,
        file_name=f'portfolio_{st.session_state.user}.zip',
...
def init_system():
    ...
    # SAFE WAY: Load keys from Environment Variables
    # You will set these in GitHub Codespaces Secrets
    GROQ_KEYS = [
    ...
    # Fallback/Check
    if not GROQ_KEYS[0]:
        st.error("API Keys missing! Please set GROQ_KEY_1 in your Environment Secrets.")
